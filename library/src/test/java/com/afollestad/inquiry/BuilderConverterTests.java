...
public class BuilderConverterTests extends BaseTest {
  ...
  public void test_basic_row_to_values() {
    ...
    List<FieldDelegate> proxiesList =
        Converter.classFieldDelegates(row.getClass(), false, BuilderRow.Builder.class);
  ...
  @Test
  public void test_cursor_to_row() throws Exception {
    ...
    when(mockInquiry.getBuilderClass(BuilderRow.class)).thenReturn((Class) builderCls);
    when(mockInquiry.getBuildMethod(BuilderRow.class, builderCls)).thenReturn(buildMethod);
