...
public class LazyLoaderList<RT> implements List<RT> {
  ...
  private void lazyLoadIfNecessary() {
    if (mDidLazyLoad || inquiry == null) return;
    if (inquiry.isDestroyed())
      throw new IllegalStateException(
          "This LazyLoaderList is not attached to a non-destroyed Inquiry instance.");
  ...
  public boolean any(@NonNull AnyPredicate<RT> predicate) {
    for (int i = 0; i < items.size(); i++) {
      if (predicate.match(items.get(i))) return true;
    }
    return false;
  }
  ...
  public boolean none(@NonNull AnyPredicate<RT> predicate) {
    for (int i = 0; i < items.size(); i++) {
      if (predicate.match(items.get(i))) return false;
    }
    return true;
  }
  ...
  public RT first(AnyPredicate<RT> predicate) {
    if (isEmpty()) return null;
    for (int i = 0; i < items.size(); i++) {
      if (predicate.match(items.get(i))) return items.get(i);
    }
    return null;
  }
