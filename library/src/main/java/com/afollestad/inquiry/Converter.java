...
class Converter {
  ...
  static String getClassTableName(Class<?> cls) {
    ...
    if (tableAnn == null)
      throw new IllegalArgumentException(
          "Class " + cls.getName() + " does not use the @Table annotation.");
    String name = tableAnn.name();
    if (name.trim().isEmpty()) name = cls.getSimpleName().toLowerCase() + "s";
    return name;
  }
   ...
   * @param inverseFieldName The optional name of a field in the child class which gets set to the
   *     parent
   * @param row The DB row
   * @param fieldType The type of field in the parent class, used to check the return type of this
   *     method
   * @param childType The type of the child class
   * @return object, array, or list depending on the fieldType
   */
  private static Object processForeignKey(
      @NonNull Inquiry inquiry,
      @NonNull String tableName,
      @NonNull String foreignColumnName,
      @Nullable String inverseFieldName,
      @NonNull Object row,
      @Nullable Class<?> fieldType,
      @NonNull Class<?> childType) {
    if (fieldType == null) {
    ...
    if (idProxy == null) {
      throw new IllegalStateException(
          "You cannot use the @ForeignKey annotation "
              + "on a field within a class that doesn't have an _id column.");
    }
    final Inquiry fkInstance =
        Inquiry.copy(inquiry, "[@fk]:" + tableName + "//" + foreignColumnName, false);

    Long rowId = idProxy.get(row);
    Object[] valuesArray =
        fkInstance.selectFrom(tableName, childType).where(foreignColumnName + " = ?", rowId).all();

    if (valuesArray == null || valuesArray.length == 0) {
      if (Utils.classImplementsList(fieldType)) return new ArrayList(0);
      else return null;
    } else {
      if (inverseFieldName != null && !inverseFieldName.isEmpty()) {
        for (Object val : valuesArray) {
          FieldDelegate inverseProxy =
              getProxyByName(classFieldDelegates(childType, false, null, false), inverseFieldName);
          if (inverseProxy == null) {
            throw new IllegalStateException(
                "Inverse field " + inverseFieldName + " not found in " + childType);
          }
          try {
  ...
  private static void loadFieldIntoRow(
      Query query,
      Cursor cursor,
      FieldDelegate proxy,
      Object row,
      int columnIndex,
      @DataType.TypeDef int columnType)
      throws Exception {
    ...
    if (fkAnn != null) {
      ...
      if (Utils.classExtendsLazyLoader(fieldType)) {
        ...
        LazyLoaderList loader =
            new LazyLoaderList(
                query.getInquiryInstance(),
                fkAnn.tableName(),
                fkAnn.foreignColumnName(),
                fkAnn.inverseFieldName(),
                row,
                childType) {
              @SuppressWarnings("unchecked")
              @Override
    ...
    switch (columnType) {
      case BLOB:
        ...
        if (blob == null) proxy.set(row, null);
        else if (fieldType == byte.class || fieldType == Byte.class) proxy.set(row, blob[0]);
        else if (fieldType == byte[].class || fieldType == Byte[].class) proxy.set(row, blob);
        else if (fieldType == Bitmap.class)
          proxy.set(row, BitmapFactory.decodeByteArray(blob, 0, blob.length));
        else proxy.set(row, deserializeObject(blob, fieldType));
        break;
      case REAL:
      ...
      case INTEGER:
        if (fieldType == short.class || fieldType == Short.class)
          ...
          throw new IllegalStateException(
              String.format(
                  "Column %s of type INTEGER " + "(decimal) doesn't match field/method of type %s",
                  columnName, fieldType.getName()));
        break;
      case TEXT:
        String text = cursor.getString(columnIndex);
        if (fieldType == String.class || fieldType == CharSequence.class) proxy.set(row, text);
          ...
          throw new IllegalStateException(
              String.format(
                  "Column %s of type TEXT " + "(string) doesn't match field/method of type %s",
                  columnName, fieldType.getName()));
        break;
    }
  }

  private static HashMap<String, FieldDelegate> buildProxyCache(
      @NonNull Class<?> cls,
  ...
  static <T> T cursorToObject(Query query, Cursor cursor, Class<T> cls) {
    ...
    final HashMap<String, FieldDelegate> cacheMap =
        buildProxyCache(cls, builderCls, foreignKeyList);
      ...
      if (buildMethod == null) {
        throw new IllegalStateException(
            "Builder "
                + builderCls.getName()
                + " must "
                + "contain a void method called build() which returns "
                + cls.getName());
      }
      try {
        ...
        throw new IllegalStateException("Failed to invoke build() of " + builderCls.getName(), e);
      }
    } else {
      return resultObject;
    }
  ...
  @Nullable
  static FieldDelegate getProxyByName(
      @Nullable List<FieldDelegate> proxyList,
      @NonNull String name,
  ...
  static List<FieldDelegate> classFieldDelegates(@NonNull Class<?> cls, boolean methodsOnly) {
    return classFieldDelegates(cls, methodsOnly, null);
  }

  static List<FieldDelegate> classFieldDelegates(
      @NonNull Class<?> cls, boolean methodsOnly, @Nullable Class<?> builderCls) {
  ...
  @Nullable
  static RowValues classToValues(
      @NonNull Object row,
      ...
      for (FieldDelegate proxy : proxiesList) {
        if (projectionArray != null && projectionArray.length > 0) {
          ...
          for (String projectionValue : projectionArray) {
            if (projectionValue != null && projectionValue.equalsIgnoreCase(proxy.name())) {
              skip = false;
              break;
            }
          }
        ...
        if (fldType.equals(String.class)) {
          ...
          throw new IllegalStateException(
              String.format(
                  "Class %s should be marked as "
                      + "Serializable, or field/method %s should use the @ForeignKey "
                      + "annotation instead of @Column.",
                  fldType.getName(), proxy.originalName()));
        }
      }

      if (columnCount == 0) {
        throw new IllegalStateException(
            "Class " + row.getClass().getName() + " " + "has no column annotated fields/methods.");
      }
      return resultValues;
    } catch (Throwable t) {
      Utils.wrapInReIfNecessary(t);
  ...
  private static byte[] serializeObject(Object obj) {
    ...
    try {
      ...
      throw new IllegalStateException(
          "Unable to serialize object of type " + obj.getClass().getName(), e);
    } finally {
      try {
