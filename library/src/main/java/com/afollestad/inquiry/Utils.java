...
class Utils {
  ...
  @CheckResult
  static <T> T newInstance(@NonNull Inquiry inquiry, @NonNull Class<T> cls) {
  ...
  private static Constructor<?> getDefaultConstructor(
      @NonNull Inquiry inquiry, @NonNull Class<?> cls) {
    if (Modifier.isAbstract(cls.getModifiers())) {
      ...
      try {
        ...
        throw new IllegalStateException(
            "Inquiry cannot construct abstract classes unless they're a top-level AutoValue class!");
      }
    }
    ...
    for (Constructor ct : constructorArray) {
      if (ct.getParameterTypes() != null && ct.getParameterTypes().length != 0) continue;
      constructor = ct;
  ...
  static String createArgsString(int argsCount) {
    ...
    sb.append("(?");
    for (int i = 1; i < argsCount; i++) sb.append(", ?");
    sb.append(')');
    return sb.toString();
  }
  ...
  static boolean classExtendsLazyLoader(Class<?> cls) {
    if (cls.equals(LazyLoaderList.class)) return true;
    while (cls.getSuperclass() != null) cls = cls.getSuperclass();
  ...
  static boolean classImplementsList(Class<?> cls) {
    ...
    for (Class i : is) if (i.equals(List.class)) return true;
    return false;
  }

  private static String getClassName(Type type) {
    String fullName = type.toString();
    if (fullName.startsWith("class ")) return fullName.substring("class ".length());
    return fullName;
  }
